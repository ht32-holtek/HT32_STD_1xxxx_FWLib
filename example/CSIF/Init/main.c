...
/*----------------------------------------------------------------------------------------------------------*/
/* The CSIF sensor data format is YVU, so 2 bytes per pixel, and CSIF Sub-sample is enabled, so the total   */
/* needs to be divided by 4(Note).                                                                          */
/*                                                                                                          */
/* Finally, the total data length is  Capture_Width * Capture_Height    * 2     /4                          */
/*                                    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   ~~~~~  ~~~~~~~~~~                    */
/*                                    The original size of a image      YUV   Sub-sample                    */
/* Note,                                                                                                    */
/*   The setting of sub-sample is as follows, which can be found in the following equation code.            */
/*   Code snippet:                                                                                          */
...
/*   Rows and columns retain 1/2 of the data, so                                                            */
/*   (Row's mask / Row's lenght) * (Column's mask / Column's Length) = (1/2) * (1/2) = 1/4                  */
/*----------------------------------------------------------------------------------------------------------*/
#define Capture_Size      Capture_Width * Capture_Height * 2 / 4
...
void CSIF_Configuration(void)
{
  ...
  CSIF_SubSampleInitStructure.CSIF_ColumnSkipMaskLength = 2;
  CSIF_SubSampleInitStructure.CSIF_RowSkipMaskLength = 2;
  CSIF_SubSampleInitStructure.CSIF_ColumnSkipMask = 0x1; /*Column's mask / Column's lenght = 1/2            */
  CSIF_SubSampleInitStructure.CSIF_RowSkipMask = 0x1;    /*Row's mask    / Row's lenght    = 1/2            */
  CSIF_SubSampleInit(&CSIF_SubSampleInitStructure);      /* Image Size / 4                                  */
...
void PDMA_Configuration(void)
{
  ...
  /* Divide by 4: Data size is 32-bit (4 bytes).                                                            */
  /* Divide by 8: Block length is 8 bytes.                                                                  */
  PDMACH_InitStructure.PDMACH_BlkCnt = Capture_Size /4 /8;
  ...
  /* Enable PDMA Transfer Complete interrupt                                                                */
