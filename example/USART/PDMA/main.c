...
void PDMA_Configuration(void)
{
  ...
  NVIC_EnableIRQ(HTCFG_TX_PDMA_IRQ);
  ...
  NVIC_EnableIRQ(HTCFG_RX_PDMA_IRQ);
  #endif
...
{
  #if 1
  if (UxART_PDMA_RxReadByte(gTempBuffer)) // Process data after 1 byted received
  {
  ...
  #else
  if (UxART_PDMA_RxGetLength() >= 5)      // Process data after 5 byted received
  {
    u32 uLen;
