/*********************************************************************************************************//**
 * @file    TM/PWMInput/ht32f1xxxx_01_it.c
 * @version $Rev:: 2642         $
 ...
 * @retval  None
 * @details In HTCFG_INPUT_PWM_TM interrupt service routine:
 *  - If the external signal under the minimum frequency,
 *    - HTCFG_INPUT_PWM_TM Update Event interrupt occurred. Reset wFrequency and wDutyCycle.
 *  - Otherwise,
 *    - The rising edge will trigger HTCFG_INPUT_PWM_INT_CHX Capture Event,
 *      HTCFG_INPUT_PWM_TM HTCFG_INPUT_PWM_INT_CHX Capture interrupt occurred.
 *    - Compute wFrequency and wDutyCycle.
 *
 ************************************************************************************************************/
void HTCFG_INPUT_PWM_IRQHandler(void)
{
  if (TM_GetIntStatus(HTCFG_INPUT_PWM_TM, TM_INT_UEV) != RESET)
  {
    /* Reset wFrequency and wDutyCycle                                                                      */
    TM_ClearIntPendingBit(HTCFG_INPUT_PWM_TM, TM_INT_UEV);
    wDutyCycle = 0;
    wFrequency = 0;
    CaptureValue = 0;
  }
  else if (TM_GetIntStatus(HTCFG_INPUT_PWM_TM, HTCFG_INPUT_PWM_INT_CHX) != RESET)
  {
    /* Compute wFrequency and wDutyCycle                                                                    */

    /* Clear HTCFG_INPUT_PWM_TM Capture compare interrupt pending bit                                                    */
    TM_ClearIntPendingBit(HTCFG_INPUT_PWM_TM, HTCFG_INPUT_PWM_INT_CHX);

    /* Get the Input Capture value                                                                          */
    CaptureValue = TM_GetCaptureCompare1(HTCFG_INPUT_PWM_TM) + SLAVE_RESTART_LATENCY;

    if (CaptureValue != 0)
    {
      u32 _SystemCoreClock = SystemCoreClock;
      /* Duty cycle computation                                                                             */
      wDutyCycle = ((TM_GetCaptureCompare0(HTCFG_INPUT_PWM_TM) + SLAVE_RESTART_LATENCY) << 16) / CaptureValue;
      wDutyCycle *= 100;

      /* wFrequency computation                                                                             */
      wFrequency = _SystemCoreClock / CaptureValue;
    }
    else
    {
      wDutyCycle = 0;
