 ...
 * @retval  None
 ************************************************************************************************************/
void HTCFG_TIME_INPUT_PWM_IRQHandler(void)
{
  bool update_flag = FALSE;

  /* store and clear all interrupt flags                                                                    */
  u32 status = HTCFG_TIME_INPUT_PWM->INTSR;
  u32 cnt = HTCFG_TIME_INPUT_PWM->CNTR;
  if ((status & TM_INT_UEV) != (HTCFG_TIME_INPUT_PWM->INTSR & TM_INT_UEV))
  {
    status = HTCFG_TIME_INPUT_PWM->INTSR;
    cnt = HTCFG_TIME_INPUT_PWM->CNTR;
  }
  HTCFG_TIME_INPUT_PWM->INTSR = ~status;
  ...
  {
    u32 cap_value = TM_GetCaptureCompare0(HTCFG_TIME_INPUT_PWM);
    ...
    TM_ChPolarityConfig(HTCFG_TIME_INPUT_PWM, TM_CH_0, CaptureCH0.ChannelPolarity);
  }

  if (status & TM_INT_CH1CC)
  {
    u32 cap_value = TM_GetCaptureCompare1(HTCFG_TIME_INPUT_PWM);
    bool isCapBeforeUpdate = (update_flag && (cap_value > cnt))? TRUE : FALSE;
    CaptureProcess(&CaptureCH1, cap_value, isCapBeforeUpdate);
    TM_ChPolarityConfig(HTCFG_TIME_INPUT_PWM, TM_CH_1, CaptureCH1.ChannelPolarity);
  }

  if (status & TM_INT_CH2CC)
  {
    u32 cap_value = TM_GetCaptureCompare2(HTCFG_TIME_INPUT_PWM);
    bool isCapBeforeUpdate = (update_flag && (cap_value > cnt))? TRUE : FALSE;
    CaptureProcess(&CaptureCH2, cap_value, isCapBeforeUpdate);
    TM_ChPolarityConfig(HTCFG_TIME_INPUT_PWM, TM_CH_2, CaptureCH2.ChannelPolarity);
  }

  if (status & TM_INT_CH3CC)
  {
    u32 cap_value = TM_GetCaptureCompare3(HTCFG_TIME_INPUT_PWM);
    bool isCapBeforeUpdate = (update_flag && (cap_value > cnt))? TRUE : FALSE;
    CaptureProcess(&CaptureCH3, cap_value, isCapBeforeUpdate);
    TM_ChPolarityConfig(HTCFG_TIME_INPUT_PWM, TM_CH_3, CaptureCH3.ChannelPolarity);
  }
}
