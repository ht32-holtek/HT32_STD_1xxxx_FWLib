...
void GPIO_OutputBit(void)
{
  GPIO_SetOutBits(HT_GPIOA, GPIO_PIN_1); // GPIO = HIGH
  __Delay(500000);

  GPIO_ClearOutBits(HT_GPIOA, GPIO_PIN_1); // GPIO = LOW
  __Delay(5000000);
}
...
void GPIO_OutputData(void)
{
  ...
  uOutputData = GPIO_ReadOutData(HT_GPIOA);
  uOutputData |= GPIO_PIN_1; // GPIO = HIGH
  GPIO_WriteOutData(HT_GPIOA, uOutputData);
  __Delay(500000);
