...
#if (LIBCFG_SPI_SCKDUTY)
/*********************************************************************************************************//**
 * @brief Set the SPIx select the SCK high/low duty mode.
 * @param SPIx: where SPIx is the selected SPI from the SPI peripherals.
 * @param SPI_SCKDutyMode:
 *   This parameter can be one of the following values:
 *     @arg SPI_SCK_HIGHDUTY_SHORTER : High-Duty is shorter than Low-Duty mode
 *     @arg SPI_SCK_HIGHDUTY_LONGER  : High-Duty is longer than Low-Duty mode
 * @retval None
 ************************************************************************************************************/
void SPI_SCKDutyConfig(HT_SPI_TypeDef* SPIx, u32 SPI_SCKDutyMode)
{
  Assert_Param(IS_SPI(SPIx));
  Assert_Param(IS_SPI_SCK_DUTY_MODE(SPI_SCKDutyMode));

  if (SPI_SCKDutyMode == SPI_SCK_HIGHDUTY_SHORTER)
  {
    SPIx->CR1 |= SPI_SCK_HIGHDUTY_SHORTER;
  }
  else
  {
    SPIx->CR1 &= SPI_SCK_HIGHDUTY_LONGER;
  }
}
#endif
/**
